...
public class SambaDocumentsProvider extends DocumentsProvider {
  ...
  private final OnTaskFinishedCallback<Uri> mLoadDocumentCallback =
      new OnTaskFinishedCallback<Uri>() {
        @Override
        public void onTaskFinished(int status, @Nullable Uri uri, Exception exception) {
  ...
  private final OnTaskFinishedCallback<DocumentMetadata> mLoadChildrenCallback =
      new OnTaskFinishedCallback<DocumentMetadata>() {
        @Override
        public void onTaskFinished( int status, DocumentMetadata metadata,
                                   Exception exception) {
  ...
  private final OnTaskFinishedCallback<String> mWriteFinishedCallback =
      new OnTaskFinishedCallback<String>() {
        ...
        public void onTaskFinished(
             int status, @Nullable String item, Exception exception) {
  ...
  public boolean onCreate() {
    System.out.println("SambaDocumentsProvider onCreate start");
    final Context context = getContext();
    AndroidApplication.init(getContext());

    mClient = AndroidApplication.getSambaClient(context);
    mCache = AndroidApplication.getDocumentCache(context);
    mTaskManager = AndroidApplication.getTaskManager(context);
    mBufferPool = new ByteBufferPool();
    mShareManager = AndroidApplication.getServerManager(context);
    mShareManager.addListener(mShareChangeListener);
    mStorageManager = (StorageManager) context.getSystemService(Context.STORAGE_SERVICE);
    mNetworkBrowser = AndroidApplication.getNetworkBrowser(context);
    System.out.println("SambaDocumentsProvider onCreate end");
    return mClient != null;
  }
            ...
            if (!isLoading && !docMap.isEmpty()) {
              LoadStatTask task = new LoadStatTask(docMap, mClient,
                  new OnTaskFinishedCallback<Map<Uri, DocumentMetadata>>() {
                    @Override
                    public void onTaskFinished(
                         int status, Map<Uri, DocumentMetadata> item, Exception exception) {
