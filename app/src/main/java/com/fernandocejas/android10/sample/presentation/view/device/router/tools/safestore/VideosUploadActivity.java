...
public class VideosUploadActivity extends BaseActivity implements View.OnClickListener {
  ...
  public void onClick(View v) {
    switch (v.getId()) {
      ...
      /*  if (isSelect()) {*/
        ...
        if (!isHasSelectFile) {
          Toast.makeText(VideosUploadActivity.this, "请先选择文件再上传", Toast.LENGTH_SHORT).show();
          return;
        }
        ...
        System.out.println("上传器 VidioUploadActivity 添加数据量 uploaders.size() = " + uploaders.size());
        ...
        while (it.hasNext()) {
          try {
            ...
            if (count < 3) {
              ...
              SambaUtils.UploadTask uploadTask = new SambaUtils.UploadTask(uploaders, VideosUploadActivity.this, mClient, UploadFragment.mHandlerUpload);
    ...
    new Thread(new Runnable() {
      ...
      public void run() {
        ...
        runOnUiThread(new Runnable() {
          @Override
          public void run() {
            adapter = new LocalFileAdapter(VideosUploadActivity.this, mLocalFileBeans, mBinding.btnUploadFile);
