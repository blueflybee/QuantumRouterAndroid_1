...
public class DownloadFinishedAdapter extends BaseAdapter {
  ...
  RemoteMainAdapter.OnDetailClickListener mOnDetailClickListener;
  ...
  public DownloadFinishedAdapter(Context context, List<FileUploadBean> fileBeans, TextView mFinishTitle) {
  ...
  // 获得文件的总数
  public int getCount() {
    System.out.println("update download 后 Adapter里面 = " + fileBeans.size());
  ...
  public void removeContainData(FileUploadBean fileUploadBean, DatabaseUtil mDBUtil) {
    ...
    if (fileBeans.contains(fileUploadBean)) {
      ...
      if (mDBUtil != null) {
        mDBUtil.deleteDownloadUrls(fileUploadBean.getPath());
      }
    }
  }
  ...
  public void add(FileUploadBean bean) {
    ...
    if (isHasSameData) {
      ...
      fileBeans.add(bean);
      mFinishTitle.setText("下载成功(" + fileBeans.size() + ")");
      notifyDataSetChanged();
    }
