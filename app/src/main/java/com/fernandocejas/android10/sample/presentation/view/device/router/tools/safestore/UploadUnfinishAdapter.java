...
//自定义Adapter内部类
public class UploadUnfinishAdapter extends BaseAdapter {
  ...
  public UploadUnfinishAdapter(Context context, TextView mLoadingTitle, Button mBtnUpload) {
  ...
  public View getView(final int position, View convertView, ViewGroup parent) {
    ...
    if (convertView == null) {
      ...
      //文件
      convertView = mLI.inflate(R.layout.item_upload_unfinish, null);
    ...
    try {
      ...
      if (fileBeans.get(position).getState() == Downloader.DOWNLOADING) {
        ...
        viewHolder.mSpeed.setText("上传失败");
      } else {
        viewHolder.mCustomCircleProgress.setStatus(CustomCircleProgress.Status.End);
  ...
  public void remove(String url) {
    ...
    for (int i = 0; i < fileBeans.size(); i++) {
      ...
      fileBeans.remove(i);
      mLoadingTitle.setText("正在上传(" + (fileBeans.size()) + ")");
      break;
    }
  ...
  public void removeContainData(FileUploadBean fileUploadBean, Map<String, Downloader> uploaders, DatabaseUtil mDBUtil1) {
    ...
    if (fileBeans.contains(fileUploadBean)) {
      ...
      if (uploaders.get(url) != null) {
        uploaders.get(url).delete(url, mContext);
      ...
      //上传完成之后移除文件
      UploadCacheManager.delete(mContext, url);
    }
  }
  ...
  public void updateUploadBtnState(Boolean isAllUpload) {
    if (isAllUpload) {
      mBtnUpload.setText("全部暂停上传");
    } else {
      mBtnUpload.setText("全部开始上传");
    }
  }
