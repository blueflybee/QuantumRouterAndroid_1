 ...
 * 兼容门锁固件版本V0.0.15
 */
public class UnlockModeActivityForV15 extends BaseActivity implements UnlockModeView {

  private static final String TAG = UnlockModeActivityForV15.class.getSimpleName();
  ...
  private void initPwdUnlockView() {
    mBinding.tvPwdUnlock.setVisibility(View.GONE);
    mBinding.pwdUnlockList.setVisibility(View.GONE);
  }

  private void initCardUnlockView() {
    mBinding.tvCardUnlock.setVisibility(View.GONE);
    mBinding.cardUnlockList.setVisibility(View.GONE);
  }
  ...
  private void configUnlockMode() {
    ...
    new Handler().postDelayed(new Runnable() {
      ...
      public void run() {
        ...
        body.setCmd(BleMapper.BLE_CMD_CONFIG_UNLOCK_MODE_V15);
        body.setPayload(mFpMode.getBleCode());
        pkg.setBody(body);
...
//        ble request failed : cmdType = GET_UNLOCK_RANDOM_CODE,reason = RESULT_FAILED,requestType = CONNECT_GATT,disconnectStatus = 133
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if (!bleRspFail(payload, true, mDeviceAddress)) {
          ...
          startBleTimer();
          mUnlockModePresenter.modifyUnlockMode(getLock().getDeviceSerialNo(), mFpMode.getUnlockModeCloud(),
              null, null, CloudUrlPath.PATH_MODIFY_UNLOCK_MODE_V15);
