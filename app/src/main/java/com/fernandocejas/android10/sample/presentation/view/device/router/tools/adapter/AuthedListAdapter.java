...
public class AuthedListAdapter extends CommAdapter1<GetWaitingAuthDeviceListResponse> {
    private Context mContext;
    public AuthedListAdapter(Context context, List<GetWaitingAuthDeviceListResponse> data, int layoutId) {
        ...
        try {
            if((!TextUtils.isEmpty(AppConstant.getMacAddress(mContext))) && (!(TextUtils.isEmpty(bean.getDev_mac())))){
                if(AppConstant.getMacAddress(mContext).equalsIgnoreCase(bean.getDev_mac())){
                    PrefConstant.IS_AUTI_DEVICE_AUTHED = true;
                    System.out.println("防蹭网：AuthedListAdapter IS_AUTI_DEVICE_AUTHED "+PrefConstant.IS_AUTI_DEVICE_AUTHED);
                }
            }
        }catch (Exception e){
            e.printStackTrace();
