 ...
 * </pre>
 */
public class ConnectWirelssUseCase extends MultipleUseCase {
  Observable<PostConnectWirelessResponse> result;
  @Inject
  public ConnectWirelssUseCase(RouterRepository routerRepository,
  ...
  @Override
  protected Observable<PostConnectWirelessResponse> buildUseCaseObservable(IRequest request) {
    QtecMultiEncryptInfo multiEncryptInfo = (QtecMultiEncryptInfo) request;

    Observable<PostConnectWirelessResponse> routerObservable =
        routerRepository.connectWireless(multiEncryptInfo.getRouterEncryptInfo())
            .subscribeOn(Schedulers.from(threadExecutor))
            .observeOn(postExecutionThread.getScheduler());

    Observable<PostConnectWirelessResponse> cloudObservable =
        cloudRepository.connectWirelessTrans(multiEncryptInfo.getCloudEncryptInfo())
            .subscribeOn(Schedulers.from(threadExecutor))
            .observeOn(postExecutionThread.getScheduler());
