 ...
 * </pre>
 */
public class GetRouterInfoUseCase extends MultipleUseCase {
  private Observable<GetRouterInfoResponse> result;

  @Inject
  public GetRouterInfoUseCase(RouterRepository routerRepository,
  ...
  @Override
  protected Observable<GetRouterInfoResponse> buildUseCaseObservable(IRequest request) {
    QtecMultiEncryptInfo multiEncryptInfo = (QtecMultiEncryptInfo) request;

    Observable<GetRouterInfoResponse> routerObservable =
        routerRepository.getRouterInfo(multiEncryptInfo.getRouterEncryptInfo())
            .subscribeOn(Schedulers.from(threadExecutor))
            .observeOn(postExecutionThread.getScheduler());

    Observable<GetRouterInfoResponse> cloudObservable =
        cloudRepository.getRouterInfoTrans(multiEncryptInfo.getCloudEncryptInfo())
            .subscribeOn(Schedulers.from(threadExecutor))
            .observeOn(postExecutionThread.getScheduler());

    //return Observable.concatDelayError(routerObservable, cloudObservable);
   // return Observable.concatDelayError(cloudObservable, routerObservable);
