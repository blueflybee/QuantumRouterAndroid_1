 ...
 * </pre>
 */
public class SetVpnUseCase extends MultipleUseCase {
  Observable<SetVpnResponse> result;
  @Inject
  public SetVpnUseCase(RouterRepository routerRepository,
  ...
  @Override
  protected Observable<SetVpnResponse> buildUseCaseObservable(IRequest request) {
    QtecMultiEncryptInfo multiEncryptInfo = (QtecMultiEncryptInfo) request;

    Observable<SetVpnResponse> routerObservable =
        routerRepository.setVpn(multiEncryptInfo.getRouterEncryptInfo())
            .subscribeOn(Schedulers.from(threadExecutor))
            .observeOn(postExecutionThread.getScheduler());

    Observable<SetVpnResponse> cloudObservable =
        cloudRepository.setVpnTrans(multiEncryptInfo.getCloudEncryptInfo())
            .subscribeOn(Schedulers.from(threadExecutor))
            .observeOn(postExecutionThread.getScheduler());
