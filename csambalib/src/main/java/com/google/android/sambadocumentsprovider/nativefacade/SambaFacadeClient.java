...
class SambaFacadeClient extends BaseClient implements SmbFacade {
  ...
  static final int USER_DATA = OPEN_FILE + 1;
  ...
  public void reset(String port) {
    try (MessageValues<Void> messageValues = MessageValues.obtain()) {
      final Message msg = obtainMessage(RESET, messageValues, port);
      enqueue(msg);
    }
  }
  ...
  public void getUserData(String port) {
    try (final MessageValues<?> messageValues = MessageValues.obtain()) {
      final Message msg = obtainMessage(USER_DATA, messageValues, port);
      enqueue(msg);
  ...
  public SmbFile openFile(String uri, String mode) throws IOException {
    System.out.println("openFile  mode = " + mode);
  ...
  private static class SambaServiceHandler extends BaseHandler {
    ...
    public void processMessage(Message msg) {
      ...
      try {
        switch (msg.what) {
          case RESET:
            System.out.println("reset port = " + uri);
            mClientImpl.reset(uri);
            break;
          case READ_DIR:
          ...
          case USER_DATA:
            System.out.println("name11111: port = " + uri);
            mClientImpl.getUserData(uri);
            break;
          default:
