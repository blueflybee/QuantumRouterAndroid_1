...
Java_com_google_android_sambadocumentsprovider_nativefacade_NativeSambaFacade_nativeInit(
 ...
 jlong
Java_com_google_android_sambadocumentsprovider_nativefacade_NativeSambaFacade_nativeInit(
        JNIEnv *env, jobject instance, jboolean debug, jlong cachePointer, jstring port_) {

    const char *port = env->GetStringUTFChars(port_, 0);
  LOGE(TAG, "Native Samba client init port = %s",port);
  ...
  const SambaClient::CredentialCache *cache =
          reinterpret_cast<SambaClient::CredentialCache *>(cachePointer);
  if (client->Init(debug, cache,port)) {
    // Successfully initialized.
    return reinterpret_cast<jlong>(client);
  } else {
    // Something wrong with the initialization.
...
Java_com_google_android_sambadocumentsprovider_nativefacade_NativeSambaFacade_nativeUseData(
        JNIEnv *env, jobject instance, jstring name_, jstring pwd_) {

    LOGE(TAG, "=============Requesting authentication data for=================== server:   and share: ");
    ...
    const SambaClient::CredentialCache *cache =
            reinterpret_cast<SambaClient::CredentialCache *>(0);
    if (client->Init(1, cache,name,pwd)) {

    }

    LOGE(TAG, "=============Requesting authentication data for=================== server:  %s and share: %s",name,pwd);
   *//* const char *name = env->GetStringUTFChars(name_, 0);
    const char *pwd = env->GetStringUTFChars(pwd_, 0);
...
void
Java_com_google_android_sambadocumentsprovider_nativefacade_NativeSambaFacade_nativeUseData(
        JNIEnv *env, jobject instance, jstring port_) {

    const char *port = env->GetStringUTFChars(port_, 0);

  /*  LOGE(TAG, "=============Requesting jString port after for=================== port: %s",port);*/
