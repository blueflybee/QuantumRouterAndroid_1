...
namespace SambaClient {
    ...
    bool SambaClient::Init(const bool debug, const CredentialCache *credentialCache,const char* port) {
        ...
        if (!sambaContext) {
            ...
            LOGD(TAG, "smb init: sambaContext  %s.", sambaContext);

        }

        LOGD(TAG, "Setting debug level to %d.", debug);
        smbc_setDebug(sambaContext, debug);
        LOGE(TAG, "Requesting authentication init  data for port: %s",
             port);
        char *newPort = strdup(port);
        LOGE(TAG, "Requesting authentication init   data for newport: %s",
             port);

        LOGD(TAG, "Setting up auth callback.");
        smbc_setOptionUserData(sambaContext,(void*)newPort);
        smbc_setFunctionAuthDataWithContext(sambaContext, GetAuthData);
        smbc_setOptionUseKerberos(sambaContext, true);
        smbc_setOptionFallbackAfterKerberos(sambaContext, true);


        int NO = 0, YES = 1;
        smbc_setTimeout(sambaContext, (int) 10000);
        smbc_setDebug(sambaContext, (int) 1);

        smbc_setOptionDebugToStderr(sambaContext, (smbc_bool) YES);
        smbc_setOptionFullTimeNames(sambaContext, (smbc_bool) YES);
        smbc_setOptionOpenShareMode(sambaContext, (smbc_share_mode) SMBC_SHAREMODE_DENY_NONE);
        smbc_setOptionSmbEncryptionLevel(sambaContext,
                                         (smbc_smb_encrypt_level) SMBC_ENCRYPTLEVEL_REQUEST);
        smbc_setOptionCaseSensitive(sambaContext, (smbc_bool) NO);
        smbc_setOptionBrowseMaxLmbCount(sambaContext, (int) 3);
        smbc_setOptionUrlEncodeReaddirEntries(sambaContext, (smbc_bool) NO);
        smbc_setOptionOneSharePerServer(sambaContext, (smbc_bool) NO);
        smbc_setOptionUseKerberos(sambaContext, (smbc_bool) NO);
        smbc_setOptionFallbackAfterKerberos(sambaContext, (smbc_bool) YES);
        smbc_setOptionNoAutoAnonymousLogin(sambaContext, (smbc_bool) NO);
        smbc_setOptionUseCCache(sambaContext, (smbc_bool) NO);
        smbc_setOptionUseNTHash(sambaContext, (smbc_bool) NO);
    ...
    void SambaClient::GetAuthData(SMBCCTX *c,
            const char *server,
                                  const char *share,
                                  char *workgroup,
                                  int maxLenWorkgroup,
        ...
        char* port = (char*)smbc_getOptionUserData(c);
        LOGE(TAG, "Requesting authentication getAuth  data for port: %s",
             port);
        char server1[64] ={0};
        if( *port != '0'){
            strcat(server1, server);
            strcat(server1,":");
            strcat(server1,port);
        } else{
            strcat(server1, server);
        }
        LOGE(TAG, "Requesting authentication getAuth  data for server1: %s",
             server1);

/*
        server = "192.168.92.61:10000";
*/

    /*    LOGV(TAG, "Requesting authentication data for server: %s and share: %s.",
             server, share);*/
       /* server = "192.168.92.61:10000";*/
        const std::string key = "smb://" + std::string(server1) + "/" + share;
...
/*
        LOGE(TAG, "Requesting authentication data for server: %s and share: %s.",
             server, share);
